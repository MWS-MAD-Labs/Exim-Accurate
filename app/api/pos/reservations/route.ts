...
export async function POST(req: NextRequest) {
  ...
  const fingerprint = crypto.createHash("sha256").update(JSON.stringify({ credentialId, payment, items })).digest("hex");
  ...
  const preferredPaymentMethod = payment.strategy === "allowance_debt"
    ? "allowance"
    : payment.strategy === "external_only"
      ? payment.method
      : "allowance";
