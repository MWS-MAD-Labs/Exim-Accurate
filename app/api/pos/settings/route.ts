...
export async function GET() {
  ...
  const settings = await prisma.posSettings.findMany({ where: { userId: session.user.id }, select: { id: true, credentialId: true, warehouseId: true, warehouseName: true, allowancePerWorkingDay: true, workingDays: true, updatedAt: true } });
...
export async function POST(req: NextRequest) {
  ...
  const { credentialId, warehouseId, warehouseName, allowancePerWorkingDay, workingDays } = parsed.data;
  ...
  try {
    ...
    const settings = await prisma.posSettings.upsert({
      where: { credentialId },
      update: { warehouseId, warehouseName, allowancePerWorkingDay, workingDays },
      create: { userId: session.user.id, credentialId, warehouseId, warehouseName, allowancePerWorkingDay, workingDays },
    });
    return NextResponse.json(settings);
  } catch {
